...
measures_region <- function(region) {
  ...
  overall <- all %>%
    select(c("region", "month", "total_pop", "count_opa", "count_pfu", "patients_opa", 
             "patients_pfu"))
  
  by_specialty <- all %>%
    select(!c("count_opa", "count_pfu", "patients_opa", "patients_pfu", "total_pop", "opa_pop"))

# Save
write.csv(overall, file = here::here("output", "processed", paste0("ts_", region, ".csv")), row.names = FALSE)
write.csv(by_specialty, file = here::here("output", "processed", paste0("ts_spec_", region, ".csv")), row.names = FALSE)

}


measures_region("london")
measures_region("northeast")
measures_region("northwest")
measures_region("yorkshire")
measures_region("eastmidlands")
measures_region("westmidlands")
measures_region("east")
measures_region("southeast")
measures_region("southwest")
